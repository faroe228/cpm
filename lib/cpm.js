...
/*
#######################################
2013-12-13 / Laurence Toenjes / lpt

cpm install dojox FAILS without my fix for cpm.js
	see: https://github.com/kriszyp/cpm/issues/20
	
Also to run under windows use my cpm batch file fix.

#######################################
*/

// ### lpt ### 12/13/2013
function getWorkingDownloadUrl( aUrl ) {
	// ### lpt Laurence Toenjes created this because cpm could not install dojox - reason corrupted archive
	// test input:           "http://packages.dojofoundation.org/archives/https$3A$2F$2Fgithub.com$2Fdojo$2Fdojox$2Fzipball$2F1.9.2";
	// test expected output: "https://github.com/dojo/dojox/zipball/1.9.2"
	
	var c1 = aUrl.indexOf("https$") >= 0;
	var c2 = aUrl.indexOf("http$")  >= 0;
	if ( c1 || c2 ) {
		var sWithPercents   = aUrl.replace( /\$/g , "%"); // replace dollar signs with percents
		var sDecoded        = decodeURIComponent( sWithPercents );
		var lastIndexOfHttp = sDecoded.lastIndexOf('http');
		return sDecoded.substring(lastIndexOfHttp);
	} else {
		return aUrl;
	}
}

var isRhino = false; // ### lpt
...
function installPackage(url, version, location, orNewer){
	...
	when(url, function(url){
		...
		when(downloadAndUnzipArchive(url, downloadPackagePath), function(){
			...
			var packageName = (installedPackageData.name || url).replace(/\.?js$/, ''); // strip off any trailing .js or js, 
...
/*				print("needsUpgrade(url) " + needsUpgrade(url) + " " + downloadPackagePath + " " + targetPackage + "/packages/" + packageName);
	...
	function downloadAndUnzipArchive(url, target){
		...
		// ### old url: http://packages.dojofoundation.org/archives/https$3A$2F$2Fgithub.com$2Fdojo$2Fdojox$2Fzipball$2F1.9.2
		// ### new url: https://github.com/dojo/dojox/zipball/1.9.2
		
		if ( isRhino ) { print( "### old url: " + url )    ; }
		if ( isRhino ) { url = getWorkingDownloadUrl( url ); } // ### lpt ###
		if ( isRhino ) { print( "### new url: " + url )    ; }
